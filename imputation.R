...
deleted_obs <- delete_obs(mice_data, 
                          var_w_missings = c("h_index", "impact_factor", 
                                             "length",
                                             "gender","journal_subject"),
                          perc = c(0.03, 0.06, 0.4, 0.21, 0.01))
...
mice_w_diag(data_missings, data_no_NA, rows,
            var_cont = c("h_index", "impact_factor", "length"),
            var_cat = c("gender", "journal_subject"),
...
comp <- rbind(eval1, eval2, eval3, eval4, eval5, eval6, eval7, eval8, eval9,
              eval10, eval11, eval12)

# make one table
comp <- mutate(as.data.frame(comp),
...
rownames(comp) <- c()
colnames(comp) <- c("Correlation\n h-index", "Correlation\n impact factor",
                    "Correlation\n length",
                    "Percentage\n gender", "Percentage\n journal subject")
comp <- cbind("method" = c("pmm", "midastouch", "rf", "cart",
